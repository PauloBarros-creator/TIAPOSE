...
class(d1[,1]) # "Date" - R data type that handles dates
#summary(db)
d1 = d1$STELLA
...
for(b in 1:Runs)  # cycle of the incremental window training (growing window)
{
  ...
  M=suppressWarnings(ets(dtr)) # create forecasting model, suppressWarnings removes warnings from HW method
  ...
  M2=fit(y~.,D[H2$tr,],model="knn") # create forecasting model
