        ...
        sidebarLayout(
          sidebarPanel(
            ...
            selectInput(inputId = "otim_model",
                        label = "Selecione modelo:",
                        choices = c("Hill Climbing","Monte Carlo","Simulated Annealing"),
                        selected = "Monte Carlo"),
            conditionalPanel(
              condition="input.otim_model == 'Hill Climbing'",
              tags$h3("Escolha da solução inicial"),
              selectInput(inputId = "init_sol",
                          label = "",
                          choices = c("Calcular Solução Inicial Aleatória","Definir Solução Inicial"),
                          selected = NULL),
              conditionalPanel(
                condition="input.init_sol =='Definir Solução Inicial'",
