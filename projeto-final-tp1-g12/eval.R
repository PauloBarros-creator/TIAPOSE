...
printeval <- function(s) {
  ...
  # output <- c(output,cat("Recursos:  ", resources, "\n"))
  # output_final <- paste(output,"\n")
  soma_v <- soma_v1+soma_v2+soma_v3
  output <- c(sales_pred1,sales_pred2,arm,soma_arm,v1,v2,v3,soma_v,preparadas1,preparadas2,head(actual_sales1,-1),head(actual_sales2,-1),profit1,profit2,tail(stock1,-1),tail(stock2,-1),sum(stock1),sum(stock2),total_profit,resources)
  return(output)
}